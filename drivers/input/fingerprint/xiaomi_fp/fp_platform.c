...
int fp_parse_dts(struct fp_device *fp_dev)
{
	...
	if (fp_dev->driver_device) {
		...
		fp_dev->pins_power_high =
			pinctrl_lookup_state(fp_dev->pinctrl, "pwr_high");
		if (IS_ERR(fp_dev->pins_power_high)) {
			ret = PTR_ERR(fp_dev->pins_power_high);
			pr_debug("%s can't find fingerprint pins_power_high\n",
				__func__);
			//return ret;
		}

		fp_dev->pins_power_low =
			pinctrl_lookup_state(fp_dev->pinctrl, "pwr_low");
		if (IS_ERR(fp_dev->pins_power_low)) {
			ret = PTR_ERR(fp_dev->pins_power_low);
			pr_debug("%s can't find fingerprint pins_power_low\n",
				__func__);
			//return ret;
		}
...
//#endif
		...
		if (IS_ERR(fp_dev->pins_power_high)) {
			status = PTR_ERR(fp_dev->pins_power_high);
			pr_debug("%s fingerprint pinctrl pins_power_high NULL\n",
				__func__);
		} else {
			pinctrl_select_state(fp_dev->pinctrl, fp_dev->pins_power_high);
			pr_debug( "power on pinctrl_select_state pins_power_high ok!\n");
		}
...
void fp_power_off(struct fp_device *fp_dev)
{
	...
	if(fp_dev->device_available == 0){
		...
		if (IS_ERR(fp_dev->pins_power_low)) {
			status = PTR_ERR(fp_dev->pins_power_low);
			pr_debug("%s fingerprint pinctrl pins_power_low NULL\n",
				__func__);
		} else {
			pinctrl_select_state(fp_dev->pinctrl, fp_dev->pins_power_low);
			pr_debug( "power on pinctrl_select_state pins_power_low ok!\n");
		}
