...
static void charger_partition_work(struct work_struct *work)
{
  ...
  for (lun = 0; lun < 6; lun++) {
    ...
    if (get_charger_partition_info()) {
      pr_info("[charger] %s get partition info ok\n", __func__);
