			...
			struct panel_event_notification *notification, void *client_data)
{
	...
	if (fp_dev && notifier_tag == PANEL_EVENT_NOTIFICATION_PRIMARY) {
		...
		case DRM_PANEL_EVENT_BLANK:
			if (fp_dev->device_available == 1) {
				...
				fp_dev->wait_finger_down = true;
				fp_netlink_send(fp_dev, FP_NETLINK_SCREEN_OFF);
				pr_err("%s FP_NETLINK_SCREEN_ON!", __func__);
			}
			break;
		...
		case DRM_PANEL_EVENT_UNBLANK:
			if (fp_dev->device_available == 1) {
				fp_dev->fb_black = 0;
				fp_netlink_send(fp_dev, FP_NETLINK_SCREEN_ON);
				pr_err("%s FP_NETLINK_SCREEN_off!", __func__);
			}
			break;
	...
	if (prim_panel) {
		...
		if (!cookie) {
			cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
					...
					goodix_fb_state_chg_callback, (void*)fp_dev);
			if (IS_ERR(cookie))
...
#endif
{
	...
	if (status < 0) {
		...
		pr_debug( "alloc_chrdev_region ok.\n" );
	}
	/* create device */
	fp_dev->device =
	...
	if (IS_ERR(fp_dev->device)) {
		...
		pr_debug( "device_create ok.\n" );
		mutex_lock(&device_list_lock);
		list_add(&fp_dev->device_entry, &device_list);
		mutex_unlock(&device_list_lock);
		pr_debug( " list_add ok.\n" );
	}
	/* cdev init and add */
	...
	if (fp_dev->input == NULL) {
		...
		pr_debug( "input_allocate_device ok.\n" );
	}
	...
	if (input_register_device(fp_dev->input)) {
		...
		pr_debug( "input_register_device ok.\n" );
	}
	...
	if(fp_dev->screen_state_wq) {
		INIT_DELAYED_WORK(&fp_dev->screen_state_dw,goodix_register_panel_notifier_work);
		queue_delayed_work(fp_dev->screen_state_wq, &fp_dev->screen_state_dw, 5 * HZ);
	}
#endif
