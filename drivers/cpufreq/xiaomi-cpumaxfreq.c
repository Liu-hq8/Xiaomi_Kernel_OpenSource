//created by caorunming@longcheer.com
...
static int cpumaxfreq_show(struct seq_file *m,void *v)
{
  ...
  //The cpumaxfreq need to be get every read time.
  //If caculated it in __init,it will be zero cause the source data hasn't been initialize.
...
{
	//mutex in seq_read_iter so no need to use it here!
...
late_initcall(proc_cpumaxfreq_init)
